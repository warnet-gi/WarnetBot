...
class LeaderboardPagination(discord.ui.View):
    ...
    async def construct_pages(self, ctx: commands.Context, leaderboard_data: List[Dict[str, Any]]) -> None:
        ...
        if self.total_page_count:
            for page_num in range(self.total_page_count):
                ...
                for member_data_list in page_member_data_list:
                    if member_data_list == page_member_data_list[1] and len(page_member_data_list[1]) == 0: continue
                    
                    field_value = ''
                    field_name = 'Rank  |  Player  |  W/L  |  ELO' if member_data_list == page_member_data_list[0] else '|'
        ...
        for embed in self.pages:
            if author_rank:
                embed.set_footer(text=f'{len(leaderboard_data)} members has been listed in this leaderboard. You are in rank #{author_rank}.')
            else:
                embed.set_footer(
    ...
    async def start(self, ctx: Interaction|commands.Context) -> None:
        ...
        self.message = await ctx.send(embed=self.pages[self.initial_page_number], view=self, ephemeral=self.ephemeral)
