...
class TCG(commands.GroupCog, group_name="warnet-tcg"):
    ...
    @app_commands.command(
        name="register",
        description="Member need to register before using other tcg commands.",
    )
    ...
    @app_commands.command(
        ...
        description="Administrator can register a member manually if they haven't registered yet.",
    )
    ...
    @app_commands.command(
        ...
        description="Administrator can unregister a member from TCG leaderboard.",
    )
    ...
    @app_commands.command(
        name="member-stats",
        description="Member can check their or someone else's TCG stats.",
    )
    @app_commands.describe(member="Member that you want to look at.")
    async def tcg_member_stats(
        self,
    ...
    @app_commands.describe(
        winner="Member who won a match.", loser="Member who lose a match."
    )
    ...
    @app_commands.command(
        ...
        description="Undo the TCG match result between players.",
    )
    @app_commands.describe(
        member="Member that you want to revert their result against other members previously.",
    )
...
async def setup(bot: WarnetBot) -> None:
    # await bot.add_cog(TCG(bot), guilds=[discord.Object(config.DEV_GUILD_ID), discord.Object(config.GUILD_ID)])
    pass
