...
class TCG(commands.GroupCog, group_name="warnet-tcg"):
    ...
    @app_commands.command(
        name='register', description='Member need to register before using other tcg commands.'
    )
    ...
    @app_commands.command(
        name='member-stats', description='Member can check their or someone else\'s TCG stats.'
    )
    @app_commands.describe(member='Member that you want to look at.')
    async def tcg_member_stats(
        self, interaction: Interaction, member: Optional[Union[discord.Member, discord.User]]
    ) -> None:
    ...
    @app_commands.command(
        name='undo-match-result', description='Undo the TCG match result between players.'
    )
    @app_commands.describe(
